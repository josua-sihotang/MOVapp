...
class PilihBangkuActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        a3.setOnClickListener {
            if (statusA3) {
                a3.setImageResource(R.drawable.ic_rect_line)
                statusA3 = false
                total -= 1
                beliTicket(total)
            } else {
                a3.setImageResource(R.drawable.ic_rect_line_selected)
                statusA3 = true
                total += 1
                beliTicket(total)
        ...
        a4.setOnClickListener {
            if (statusA4) {
                a4.setImageResource(R.drawable.ic_rect_line)
                statusA4 = false
                total -= 1
                beliTicket(total)
            } else {
                a4.setImageResource(R.drawable.ic_rect_line_selected)
                statusA4 = true
                total += 1
                beliTicket(total)
    ...
    private fun beliTicket(total: Int) {
        if (total==0){
